...
public class Question2 extends SubsystemBase {
    ...
    private final MotionMagicVoltage m_test_motor_request1 = new MotionMagicVoltage(0);
    private final MotionMagicVoltage m_test_motor_request2 = new MotionMagicVoltage(0);

    public void setmotorPosition(double Position, double Position2) {
        m_test_motor.setControl(m_test_motor_request1.withPosition(Position));
        m_test_motor2.setControl(m_test_motor_request2.withPosition(Position2));
      }
    ...
    public boolean isAtPosition(double expected_position){
      Constants.MOTORinQ1.current_position = m_test_motor.getPosition().getValueAsDouble();
      System.out.println("current_position" + Constants.MOTORinQ1.current_position );
      return Math.abs(Constants.MOTORinQ1.current_position - expected_position) <= Constants.MOTORinQ1.expected_error;
    }
    public boolean isAtPosition2(double expected_position2){
      Constants.MOTORinQ1.current_position2 = m_test_motor2.getPosition().getValueAsDouble();
      return Math.abs(Constants.MOTORinQ1.current_position2 - expected_position2) <= Constants.MOTORinQ1.expected_error;
    }
